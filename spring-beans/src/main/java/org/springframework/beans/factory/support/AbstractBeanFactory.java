...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(final String name, @Nullable final Class<T> requiredType, @Nullable final Object[] args, boolean typeCheckOnly) throws BeansException {
		...
		/**
		 * 从缓存中获取 Bean。代码见 org.springframework.beans.factory.support.DefaultSingletonBeanRegistry#getSingleton(java.lang.String, boolean)。
		 * 具体步骤：
		 *          1. 从一级缓存 singletonObjects 中获取 bean。
		 *          2. 一级缓存中 没有 ！ 并且 当前 bean 正在被 创建，从二级缓存 earlySingletonObjects 中获取。
		 *          3. 如果 二级缓存 也没有 ! 切 允许 提前引用 ！ 从 三级缓存 singletonFactories 中获取。
		 *          4. 如果三级缓存 中也没有，直接返回空。
		 *          5. 如果三级缓存中有， 调用 singletonFactory.getObject() 方法获取 bean。同时，把 三级缓存 中的 对象 ，放入 二级缓存。从 三级缓存 中 删除 该对象。
		 *          6. 至此， 就从三级缓存升级到二级缓存了。
		 *
		 */
...
//			}
			...
			// 检查 BeanDefinition 是否已经在 BeanFactory.beanDefinitionMap 中存在
	...
	/**
	 * 返回一个 RootBeanDefinition。如果是 BeanDefinition 的子类，合并父类和自身的属性。
	 * <p>
